...
const jsCode = "window.postMessage(document.getElementsByTagName('html').innerHTML);";
...
class Map extends Component {
  ...
  async parseContentToRoute(responseData, callback) {
    ...
    for (let i in finalResult) {
      ...
      if (finalCoords.length == finalResult.length){
        // finalCoords.map(function(el, index){
        //   finalCoords[index]['distance'] = self.calculateDistance(finalCoords[0].latitude,finalCoords[0].longitude,finalCoords[index].latitude,finalCoords[index].longitude,"K");
        // });
        // finalCoords.sort(function(a, b) { 
        //   return a.distance - b.distance;
        // });
        // console.log(JSON.stringify(finalCoords))
  ...
  loadRoute = (item) => {
    const self = this;
    this.setState({loading: true, selectedLine: item, stopDetail: null});
    this.hideStopDetails();
    Actions.refresh({title: item.title});
    self.setState({bgUri: domain+'/'+item.id});
  ...
  parseHtmlForStop(item) {
        ...
        for (let k in layerContent[i].attributes) {
          if (layerContent[i].attributes[k].name === 'onclick') {
            if (String(layerContent[i].attributes[k].nodeValue).indexOf('Linea')!=-1) {
              ...
              isChild = true;
              // link = String(layerContent[i].attributes[k].nodeValue).replace('document.location=', '');
            }
            break;
          }
        }
    ...
    // req.open("GET", domain+'/'+item.link.replace('DisplayParadas', 'lineas'), true);
    ...
    //     // let regex = new RegExp('id="ctl00_ContentPlaceHolder2_TreeView1n0Nodes"(.*?)id="ctl00_ContentPlaceHolder2_TreeView1_SkipLink"', 'gm');
    //     // let contentRegex = result.match(regex);
    //     // result = result.replace(/display:none;/g, 'display:block;');
    //     let htmlToParse = parser.parseFromString(result, 'text/xhtml');
    //     console.log(htmlToParse)
    //     let layerContent = htmlToParse.getElementById('ctl00_ContentPlaceHolder2_TreeView1');
