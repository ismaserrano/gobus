...
class AuthForm extends Component {
  ...
  static defaultProps = {
    ...
    buttonTitle: 'Entrar',
    successMessage: 'Genial! has accedido con éxito',
    introTitle: null,
    introText: null,
  }
    ...
    this.state = {
      ...
      options: {
        fields: {
          ...
          LastName: {
            template: TcombTextInput,
            error: 'Por favor, introduce tu apellido',
            clearButtonMode: 'while-editing',
          },
        },
      },
    ...
    this.setState({
      options: FormValidation.update(this.state.options, {
        fields: {
          ConfirmPassword: {
            hasError: { $set: error },
            error: { $set: error ? 'Las contraseñas no coinciden!' : '' },
          },
        },
      }),
      form_values: form,
  ...
  handleSubmit = () => {
    ...
    if (formData) {
      this.setState({ form_values: formData }, () => {
        this.setState({ resultMsg: { status: 'Un momento...' } });
        ...
        <Card>
          ...
          {this.props.formType === 'login' &&
            <View>
              ...
              <Button outlined title={'Entrar'} onPress={Actions.signUp} />
            </View>
          }
        </Card>
